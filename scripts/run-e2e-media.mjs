  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (res.ok && isHealthyHtml(text)) {
        console.log(`[e2e-media] ready: ${label}`);
        return;
      }
    } catch {
      // retry
...
async function main() {
  ...
  try {
    ...
    const groupId = process.env.E2E_GROUP_CHAT_ID;
    if (groupId) {
      await waitForHealthy(`${baseUrl}/chat/${groupId}`, `chat/${groupId}`, deadline);
    }
  } catch (e) {
