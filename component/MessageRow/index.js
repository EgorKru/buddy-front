...
const MessageRow = React.memo(
  ({
    ...
    return (
      <div key={msg.id}>
        {showDate && <div className={styles.dateDivider}>{formatChatDate(msg.createdAt)}</div>}
        <div
          ref={messageRef}
          ...
          <div className={styles.messageContent}>
            {msg.type === 'SYSTEM' && msg.content && msg.content.includes('вызов') ? (
              <>
                ...
                <SystemCallMessage message={msg} chatId={msg.chatId} chats={chats} />
              </>
            ) : (
              <>
                {!isOwn && (
                ...
                {msg.type === 'VOICE' && msg.fileUrl ? (
                  ...
                  >
                    ...
                    <div className={styles.messageTextMeta}>
                      ...
                      {msg.edited && (
                        <span
                          className={styles.messageEdited}
                          title={
                            msg.editedAt
                              ? `Отредактировано ${formatChatTime(msg.editedAt)}`
                              : 'Отредактировано'
                          }
                        >
                          (ред.)
                      ...
                      {statusIcon && (
                        <span
                          title={
                            readMeta?.readCount
                              ? readMeta.totalOthers > 1
                                ? `Прочитали ${readMeta.readCount}/${readMeta.totalOthers}`
                                : 'Прочитано'
                              : 'Отправлено'
                          }
                        >
