  ...
  const handleSearch = useCallback(
    async (query, pageNum = 0) => {
      ...
      try {
        ...
        const results = Array.isArray(response?.content)
          ? response.content
        ...
        if (upsertMessage) {
          for (const msg of sortedResults) {
            ...
            upsertMessage(
              { ...msg, status: MESSAGE_STATUS.SENT, isOptimistic: false },
              { unreadDelta: 0 }
            );
          }
        }
        ...
        setHasMoreSearchResults(
          response?.totalPages
            ...
            : results.length === MESSAGE_PAGE_SIZE
        );
        setSearchMode(true);
  ...
  const handleNavigateToSearchResult = useCallback(
    async (messageId) => {
      if (!onNavigateToMessage || !messageId) {
        console.error('handleNavigateToSearchResult: missing onNavigateToMessage or messageId', {
          onNavigateToMessage,
          messageId,
        });
        return;
